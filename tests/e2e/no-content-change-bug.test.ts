...
describe('No Content Change Update Bug Investigation', () => {
  it('should not cause duplication when updating without content changes', async () => {
    ...
    const initialPropertiesCount = (originalContent.match(/:PROPERTIES:/g) || []).length;
    const initialEndCount = (originalContent.match(/:END:/g) || []).length;
    const initialTitleCount = (originalContent.match(/^#\+title:/gm) || []).length;
    
    console.log(`Initial counts - PROPERTIES: ${initialPropertiesCount}, :END: ${initialEndCount}, #+title: ${initialTitleCount}`);
    ...
    const identicalUpdateResponse = await ApiHelpers.updateNode(initialNode.id, identicalUpdateData);
    console.log('Identical update response status:', identicalUpdateResponse.status);
    expect(identicalUpdateResponse.status).toBe(200);
    
    // Step 4: Check content after identical update
    const afterIdenticalContent = await ApiHelpers.getNodeContent(initialNode.id);
    ...
    const afterIdenticalPropertiesCount = (afterIdenticalText.match(/:PROPERTIES:/g) || []).length;
    const afterIdenticalEndCount = (afterIdenticalText.match(/:END:/g) || []).length;
    const afterIdenticalTitleCount = (afterIdenticalText.match(/^#\+title:/gm) || []).length;
    
    console.log(`After identical update counts - PROPERTIES: ${afterIdenticalPropertiesCount}, :END: ${afterIdenticalEndCount}, #+title: ${afterIdenticalTitleCount}`);
    
    if (afterIdenticalPropertiesCount > 1) {
      console.error('BUG DETECTED: PROPERTIES duplication after identical update!');
    }
    if (afterIdenticalTitleCount > 1) {
      console.error('BUG DETECTED: #+title duplication after identical update!');
    }
    ...
    const partialUpdateResponse = await ApiHelpers.updateNode(initialNode.id, partialUpdateData);
    console.log('Partial update response status:', partialUpdateResponse.status);
    ...
    const afterPartialPropertiesCount = (afterPartialText.match(/:PROPERTIES:/g) || []).length;
    const afterPartialEndCount = (afterPartialText.match(/:END:/g) || []).length;
    const afterPartialTitleCount = (afterPartialText.match(/^#\+title:/gm) || []).length;
    
    console.log(`After partial update counts - PROPERTIES: ${afterPartialPropertiesCount}, :END: ${afterPartialEndCount}, #+title: ${afterPartialTitleCount}`);
    
    if (afterPartialPropertiesCount > 1) {
      console.error('BUG DETECTED: PROPERTIES duplication after partial update!');
    }
    ...
    const minimalUpdateResponse = await ApiHelpers.updateNode(initialNode.id, minimalUpdateData);
    console.log('Minimal update response status:', minimalUpdateResponse.status);
    ...
    const finalPropertiesCount = (finalText.match(/:PROPERTIES:/g) || []).length;
    const finalEndCount = (finalText.match(/:END:/g) || []).length;
    const finalTitleCount = (finalText.match(/^#\+title:/gm) || []).length;
    
    console.log(`Final counts - PROPERTIES: ${finalPropertiesCount}, :END: ${finalEndCount}, #+title: ${finalTitleCount}`);
  ...
  it('should preserve exact content when no changes are made', async () => {
    ...
    console.log('Original content preview (first 200 chars):', originalContent.substring(0, 200));
    ...
    const updateResponse = await ApiHelpers.updateNode(testNode.id, preservationUpdate);
    ...
    console.log('Updated content preview (first 200 chars):', updatedContent.substring(0, 200));
  ...
  it('should handle rapid sequential updates without duplication', async () => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      console.log(`After rapid update ${i} - PROPERTIES: ${propertiesCount}, #+title: ${titleCount}`);
      
      if (propertiesCount > 1) {
        console.error(`BUG DETECTED: PROPERTIES duplication after rapid update ${i}!`);
