...
describe("Metadata Duplication Bug Investigation", () => {
  it("should test for PROPERTIES duplication in org files during updates", async () => {
    ...
    if (initialContent.status === 200) {
      ...
      console.log(
        "Initial content preview (first 300 chars):",
      ...
      console.log(
        ...
        propertiesMatches ? propertiesMatches.length : 0,
      );
      console.log(
        "Initial :END: count:",
        endPropertiesMatches ? endPropertiesMatches.length : 0,
      );
      console.log(
        "Initial #+title count:",
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      if (updatedContent.status === 200) {
        ...
        console.log(
          `After update ${i} - content preview (first 400 chars):`,
        ...
        console.log(
          `After update ${i} - PROPERTIES count:`,
          propertiesMatches ? propertiesMatches.length : 0,
        );
        console.log(
          `After update ${i} - :END: count:`,
          endPropertiesMatches ? endPropertiesMatches.length : 0,
        );
        console.log(
          `After update ${i} - #+title count:`,
          titleMatches ? titleMatches.length : 0,
        );
        console.log(
          `After update ${i} - #+category count:`,
          categoryMatches ? categoryMatches.length : 0,
        );

        if (propertiesMatches && propertiesMatches.length > 1) {
          console.error(
            `BUG DETECTED: PROPERTIES duplication after update ${i}!`,
          );
        ...
        if (categoryMatches && categoryMatches.length > 1) {
          console.error(
            `BUG DETECTED: #+category duplication after update ${i}!`,
          );
  ...
  it("should test for YAML front matter duplication in markdown files during updates", async () => {
    ...
    if (initialContent.status === 200) {
      const content = initialContent.body.content || "";
      console.log(
        "Initial content preview (first 400 chars):",
        content.substring(0, 400),
      );
      ...
      console.log(
        "Initial YAML start (---) count:",
        yamlStartMatches ? yamlStartMatches.length : 0,
      );
      console.log(
        "Initial title: count:",
        titleMatches ? titleMatches.length : 0,
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      console.log(
        `Markdown update ${i} response status:`,
      ...
      if (updatedContent.status === 200) {
        const content = updatedContent.body.content || "";
        console.log(
          `After markdown update ${i} - content preview (first 500 chars):`,
        ...
        console.log(
          `After markdown update ${i} - YAML start (---) count:`,
          yamlStartMatches ? yamlStartMatches.length : 0,
        );
        console.log(
          `After markdown update ${i} - title: count:`,
          titleMatches ? titleMatches.length : 0,
        );
        console.log(
          `After markdown update ${i} - id: count:`,
          idMatches ? idMatches.length : 0,
        );
        console.log(
          `After markdown update ${i} - category: count:`,
          categoryMatches ? categoryMatches.length : 0,
        );
        console.log(
          `After markdown update ${i} - roam_aliases: count:`,
          aliasMatches ? aliasMatches.length : 0,
        );
        console.log(
          `After markdown update ${i} - roam_refs: count:`,
          refMatches ? refMatches.length : 0,
        );

        if (yamlStartMatches && yamlStartMatches.length > 2) {
          console.error(
            `BUG DETECTED: YAML front matter duplication after markdown update ${i}!`,
          );
          console.error(
            `YAML delimiters found: ${yamlStartMatches.length} (expected 2)`,
          );
        }

        if (titleMatches && titleMatches.length > 1) {
          console.error(
            `BUG DETECTED: title: duplication after markdown update ${i}!`,
          );
          console.error(`title: lines found: ${titleMatches.length}`);
        }
        ...
        if (idMatches && idMatches.length > 1) {
          console.error(
            `BUG DETECTED: id: duplication after markdown update ${i}!`,
          );
        ...
        if (categoryMatches && categoryMatches.length > 1) {
          console.error(
            `BUG DETECTED: category: duplication after markdown update ${i}!`,
          );
          console.error(`category: lines found: ${categoryMatches.length}`);
        }
  ...
  it("should test for metadata duplication with concurrent updates", async () => {
    ...
    if (orgContent.status === 200) {
      ...
      console.log(
        "After concurrent updates - Org PROPERTIES count:",
        propertiesMatches ? propertiesMatches.length : 0,
      );
      console.log(
        "After concurrent updates - Org #+title count:",
        titleMatches ? titleMatches.length : 0,
      );
    ...
    if (mdContent.status === 200) {
      ...
      console.log(
        "After concurrent updates - MD YAML delimiters count:",
        yamlStartMatches ? yamlStartMatches.length : 0,
      );
      console.log(
        "After concurrent updates - MD title: count:",
        titleMatches ? titleMatches.length : 0,
      );
  ...
  it("should test for partial update metadata integrity", async () => {
    ...
    if (updatedContent.status === 200) {
      const content = updatedContent.body.content || "";
      console.log(
        "After partial update - content preview:",
      ...
      console.log(
        "After partial update - YAML delimiters:",
        yamlStartMatches ? yamlStartMatches.length : 0,
      );
      console.log(
        "After partial update - title lines:",
        titleMatches ? titleMatches.length : 0,
      );
      console.log(
        "After partial update - id lines:",
        idMatches ? idMatches.length : 0,
      );
