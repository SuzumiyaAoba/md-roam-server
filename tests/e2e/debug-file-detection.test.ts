...
describe.skip('Debug File Detection Issues', () => {
  it('should debug why files exist but nodes endpoint returns empty', async () => {
    ...
    if (response.body.data && Array.isArray(response.body.data)) {
      ...
      if (!foundNode) {
        ...
        console.error('All returned node IDs:', response.body.data.map((n: any) => n.id));
      }
    } else {
    ...
    console.log('Second attempt - Data length:', 
      Array.isArray(secondResponse.body.data) ? secondResponse.body.data.length : 'not array');
    
    if (secondResponse.body.data && Array.isArray(secondResponse.body.data)) {
      const foundNodeSecond = secondResponse.body.data.find((node: any) => node.id === testNode.id);
      console.log('Node found in second attempt:', foundNodeSecond ? 'YES' : 'NO');
    }
    ...
    try {
      ...
      console.log('Individual retrieval success:', individualResponse.status === 200 ? 'YES' : 'NO');
    } catch (err) {
  ...
  it('should check database synchronization timing', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const response = await ApiHelpers.getAllNodes();
      const foundCount = nodes.filter(n => 
        response.body.data && 
        Array.isArray(response.body.data) && 
        response.body.data.find((apiNode: any) => apiNode.id === n.id)
      ).length;
      
      console.log(`After creating ${i + 1} nodes, found ${foundCount} in API response`);
    }
    
    // Final check
    const finalResponse = await ApiHelpers.getAllNodes();
    const finalFoundCount = nodes.filter(n => 
      finalResponse.body.data && 
      Array.isArray(finalResponse.body.data) && 
      finalResponse.body.data.find((apiNode: any) => apiNode.id === n.id)
    ).length;
    
    console.log(`Final check: Created ${nodes.length} nodes, found ${finalFoundCount} in API`);
    
    if (finalFoundCount !== nodes.length) {
      console.error('SYNC TIMING ISSUE: Not all created nodes appear in API response');
    }
