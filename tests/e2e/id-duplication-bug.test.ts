...
describe('ID Duplication Bug Investigation', () => {
  it('should test for ID duplication during node updates', async () => {
    ...
    console.log('After update node count:', afterUpdateList.body.data?.length || 0);
    
    const afterUpdateIds = afterUpdateList.body.data?.map((n: any) => n.id) || [];
    ...
    if (duplicatedIds.length > 0) {
      ...
      duplicatedIds.forEach(([id, count]) => {
        ...
        const nodesWithSameId = afterUpdateList.body.data?.filter((n: any) => n.id === id);
    ...
    const updatedNodes = afterUpdateList.body.data?.filter((n: any) => n.id === initialNode.id) || [];
    console.log(`Nodes with original ID ${initialNode.id}:`, updatedNodes.length);
  ...
  it('should test for ID duplication with multiple sequential updates', async () => {
    ...
    for (let i = 1; i <= updateCount; i++) {
      ...
      const nodesWithSameId = nodeList.body.data?.filter((n: any) => n.id === testNode.id) || [];
    ...
    expect(finalNodesWithId[0].title).toBe(`Sequential Update Test - Update ${updateCount}`);
  ...
  it('should test for ID duplication with concurrent updates', async () => {
    ...
    const concurrentUpdates = [
      ApiHelpers.updateNode(testNode.id, { title: 'Concurrent Update 1', content: 'Content 1' }),
      ApiHelpers.updateNode(testNode.id, { title: 'Concurrent Update 2', content: 'Content 2' }),
      ApiHelpers.updateNode(testNode.id, { title: 'Concurrent Update 3', content: 'Content 3' })
    ];
    ...
    responses.forEach((response, index) => {
      if (response.status === 'fulfilled') {
        console.log(`Concurrent update ${index + 1} succeeded:`, response.value.status);
  ...
  it('should detect database sync issues causing apparent duplication', async () => {
    ...
    const allNodeCounts = rapidCalls.map(response => response.body.data?.length || 0);
    const allNodeIds = rapidCalls.map(response => response.body.data?.map((n: any) => n.id) || []);
    ...
    const consistentCounts = allNodeCounts.every(count => count === firstCount);
    
    if (!consistentCounts) {
      console.error('BUG DETECTED: Inconsistent node counts across rapid API calls');
      console.error('Node counts:', allNodeCounts);
    }
    
    // Check for our specific node in each response
    const ourNodeCounts = allNodeIds.map(ids => ids.filter((id: string) => id === testNode.id).length);
    ...
    if (!consistentOurNode) {
      console.error('BUG DETECTED: Inconsistent node presence across rapid API calls');
