...
*** TODO 重要なタスク`;
    ...
    syntaxChecks.forEach((check, index) => {
      ...
      if (found) {
        console.log(
          `✅ ${(index + 1).toString().padStart(2, " ")}. ${check.name} - 見つかりました`,
        );
        passedChecks++;
      } else {
        console.log(
          `❌ ${(index + 1).toString().padStart(2, " ")}. ${check.name} - 見つかりません`,
        );
        failedChecks++;
      }
    });
...
- 矢印: → ← ↑ ↓ ↔ ⇒`;
    ...
    console.log(
      `エッジケース成功率: ${Math.round(edgeSuccessRate * 100)}% (${edgePassedChecks}/${edgeCaseChecks.length})`,
    );
