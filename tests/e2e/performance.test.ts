  ...
  describe("Individual Operation Performance", () => {
    it("should create nodes within performance threshold", async () => {
      const testCases = [
        ...
        { name: "Rich Org", data: EXTENDED_TEST_NODES.formatting[1] },
        {
          name: "Rich Text",
          data: {
            title: "Rich Text Performance",
            content: "# Header\n\nRich **bold** and *italic* text.",
            tags: ["performance", "rich"],
            file_type: "md" as const,
          },
        },
      ];
    ...
    it("should retrieve nodes within performance threshold", async () => {
      ...
      const testNodes = await Promise.all([
        TestCleanup.createTestNode({
          title: "Retrieval Test 1",
          file_type: "md",
        }),
        TestCleanup.createTestNode({
          title: "Retrieval Test 2",
          file_type: "org",
        }),
        TestCleanup.createTestNode({
    ...
    it("should search within performance threshold", async () => {
      ...
      const searchableNodes = await Promise.all([
        ...
        TestCleanup.createTestNode({
          title: "Performance Search Test 2",
          tags: ["performance"],
        }),
        TestCleanup.createTestNode({
          title: "Performance Test Node",
      ...
      const searchQueries = ["performance", "test", "content", "search"];
      ...
      for (const query of searchQueries) {
        ...
        console.log(
          `Search "${query}": ${duration}ms (${response.body.results.length} results)`,
        );
      }
    });
  });
  ...
  describe("Bulk Operations Performance", () => {
    it("should handle bulk node creation efficiently", async () => {
      ...
      for (const size of bulkSizes) {
        ...
        console.log(
          `Bulk creation (${size} nodes): ${duration}ms total, ${avgTimePerNode.toFixed(2)}ms per node`,
        );
      }
    });
    ...
    it("should handle bulk retrieval efficiently", async () => {
      ...
      const duration = endTime - startTime;
      const successfulRetrievals = responses.filter(
        (r) => r.status === 200,
      ).length;
      ...
      console.log(
        `Bulk retrieval (${testNodes.length} nodes): ${duration}ms total, ${avgTimePerRetrieval.toFixed(2)}ms per node`,
      );
    });
    ...
    it("should handle bulk updates efficiently", async () => {
      ...
      console.log(
        `Bulk update (${testNodes.length} nodes): ${duration}ms total, ${avgTimePerUpdate.toFixed(2)}ms per node`,
      );
    });
  });
  ...
  describe("Concurrent Operations Performance", () => {
    it("should handle concurrent node creation", async () => {
      ...
      expect(duration).toBeLessThan(
        PERFORMANCE_THRESHOLDS.concurrentOperations,
      );
      ...
      console.log(
        `Concurrent creation (${concurrentCount} nodes): ${duration}ms, ${successfulCreations.length} successful`,
      );
    });
    ...
    it("should handle mixed concurrent operations", async () => {
      ...
      const duration = endTime - startTime;
      const successfulOps = responses.filter(
      ...
      expect(duration).toBeLessThan(
        PERFORMANCE_THRESHOLDS.concurrentOperations * 1.5,
      ); // Allow more time
      ...
      console.log(
        `Mixed concurrent ops (${operations.length} operations): ${duration}ms, ${successfulOps} successful`,
      );
    });
  });
  ...
  describe("Large Data Performance", () => {
    it("should handle large content efficiently", async () => {
      ...
      for (const size of contentSizes) {
        const largeContent = "Large content block. ".repeat(
          Math.ceil(size / 20),
        );
        ...
        if (response.status === 201) {
          ...
          expect(retrievalDuration).toBeLessThan(
            PERFORMANCE_THRESHOLDS.nodeRetrieval * 3,
          ); // Allow 3x normal time for large content

          console.log(
            `Large content (${size} chars): creation ${duration}ms, retrieval ${retrievalDuration}ms`,
          );
        } else {
          console.log(
            `Large content (${size} chars) failed: ${response.status} - ${response.body?.message}`,
          );
        }
    ...
    it("should handle many metadata items efficiently", async () => {
      ...
      for (const size of metadataSizes) {
        ...
        if (response.status === 201) {
          ...
          console.log(
            `Many metadata (${size} tags) failed: ${response.status}`,
          );
        }
  ...
  describe("System Resource Usage", () => {
    it("should maintain performance under sustained load", async () => {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, loadTestDuration + 1000),
      );
      ...
      console.log(
        `Sustained load: ${operationCount} operations in ${totalDuration}ms, ${successfulOps} successful`,
      );
    ...
    it("should handle memory-intensive operations", async () => {
      ...
      const memoryIntensiveNodes = [
        ...
        },
        {
          ...
          content: "Pattern: English test content with repeated data ".repeat(
            200,
          ), // Safe English content, reduced repetition
          file_type: "org" as const,
        },
      ];
      ...
      console.log(
        `Memory-intensive operations: ${duration}ms, ${successfulNodes.length} successful`,
      );
