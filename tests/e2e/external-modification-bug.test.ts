...
describe('External File Modification Bug Investigation', () => {
  it('should handle updates after external file modifications', async () => {
    ...
    const externallyModifiedContent = currentContent + '\n\n* Externally Added Section\n\n** Subsection\n\nSome external content.';
    ...
    console.log(`Final counts - PROPERTIES: ${propertiesCount}, :END: ${endCount}, #+title: ${titleCount}`);
    
    if (propertiesCount > 1) {
      console.error('BUG DETECTED: PROPERTIES duplication after external modification + API update!');
    }
    if (titleCount > 1) {
      console.error('BUG DETECTED: #+title duplication after external modification + API update!');
    }
...
* Additional Content`;
    ...
    console.log(`After sync+update counts - PROPERTIES: ${propertiesCount}, #+title: ${titleCount}`);
  ...
  it('should detect when existing files have duplication', async () => {
    ...
    console.log(`Checking ${allNodes.body.data.length} existing nodes for duplication...`);
      ...
      try {
        ...
        if (content.status === 200 && content.body.content) {
          ...
          if (propertiesCount > 1 || titleCount > 1) {
            ...
            console.log(`  - PROPERTIES: ${propertiesCount}, #+title: ${titleCount}`);
    ...
    if (duplicatedNodes.length > 0) {
      ...
      duplicatedNodes.forEach((node, index) => {
        ...
        console.log(`   PROPERTIES: ${node.propertiesCount}, #+title: ${node.titleCount}`);
      });
    } else {
