...
export class AdvancedSearchService {
  ...
  private parseMarkdownNode(
    ...
    const metadata: any = { tags: [], aliases: [], refs: [] };
    let id = '';
    let title = '';
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('id:')) {
        ...
        if (aliasesMatch) {
          metadata.aliases = aliasesMatch[1].split(',').map(a => a.trim().replace(/['"]/g, ''));
        }
      ...
      for (const field of request.fields) {
        ...
        if (fieldValue) {
          ...
          if (similarity >= request.threshold) {
            const distance = this.calculateLevenshteinDistance(normalizedQuery, normalizedValue);
  ...
  async phraseSearch(request: PhraseSearchRequest): Promise<{
    ...
    for (const node of nodes) {
      const matches: Array<{ field: string; snippet: string; position: number }> = [];
      ...
      for (const field of request.fields) {
        ...
        if (fieldValue) {
          const searchText = request.caseSensitive ? fieldValue : fieldValue.toLowerCase();
          const position = searchText.indexOf(searchPhrase);
          
          if (position !== -1) {
            // Create snippet around the match
            const snippetStart = Math.max(0, position - 50);
            const snippetEnd = Math.min(fieldValue.length, position + searchPhrase.length + 50);
      ...
      if (fieldValue) {
        ...
        if (request.exact) {
          ...
          if (!isMatch) {
            ...
            const similarity = this.calculateSimilarity(searchQuery, searchText);
            if (similarity >= 0.7) {
              isMatch = true;
              matchType = "fuzzy";
            }
    ...
    for (const node of nodes) {
      ...
      switch (request.field) {
        ...
        case 'content':
          ...
          candidates = node.content.split(/\s+/).filter(word => 
      ...
      for (const candidate of candidates) {
        ...
        if (candidateLower.includes(queryLower) && !suggestionSet.has(candidateLower)) {
          suggestionSet.add(candidateLower);
          
          // Calculate relevance score
          const similarity = this.calculateSimilarity(queryLower, candidateLower);
  ...
  async highlightSearch(request: HighlightSearchRequest): Promise<{
    ...
    for (const node of nodes) {
      ...
      for (const field of fields) {
        ...
        const searchText = request.caseSensitive ? field.value : field.value.toLowerCase();
      ...
      if (highlights.length > 0) {
        ...
        if (contentHighlight && contentHighlight.positions.length > 0) {
          ...
          const start = Math.max(0, firstMatch.start - request.snippetLength / 2);
          const end = Math.min(
