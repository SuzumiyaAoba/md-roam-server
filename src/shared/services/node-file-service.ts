...
export class NodeFileService {
  ...
  private sanitizePartialNodeData(
    data: Partial<CreateNodeData>,
  ): Partial<CreateNodeData> {
    const sanitize = (str: string): string => {
      // Basic XSS prevention: remove script tags and dangerous HTML
      return str
        .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "")
  ...
  private generateId(): string {
    ...
    const _random = Math.random().toString(16).substr(2, 8);
  ...
  createNode(data: CreateNodeData): NodeInfo {
    ...
    try {
      ...
      throw new Error(
        `Failed to create node file: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  updateNode(id: string, data: Partial<CreateNodeData>): NodeInfo | null {
    ...
    try {
      ...
      const updatedData = this.mergeNodeData(
        content,
        sanitizedData,
      ...
      try {
        ...
        if (!updatedContent || !updatedContent.includes(id)) {
          throw new Error(
            `File update verification failed for ${existingFile.path}`,
          );
        }
      } catch (error) {
        throw new Error(
          `Update verification failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
  ...
  private findFileById(id: string): NodeInfo | null {
    ...
    for (const file of files) {
      ...
      if (fileType === "md") {
        ...
        hasId = Boolean(idMatch && idMatch[1] === id);
      } else {
        // Look for :ID: ID in org properties
        const idMatch = content.match(/^:ID:\s*(.+)$/m);
        hasId = Boolean(idMatch && idMatch[1]?.trim() === id);
      }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith("#+title:")) {
        ...
        for (let j = 0; j < i; j++) {
          ...
          if (
            prevLine.startsWith(":PROPERTIES:") ||
            prevLine.startsWith("#+")
          ) {
            foundMetadata = true;
            break;
  ...
  getStats(): {
    ...
    for (const node of allNodes) {
      try {
        ...
        totalAliases += parsed.aliases?.length ?? 0;
        totalRefs += parsed.refs?.length ?? 0;

        // Count citations (simplified - look for [@citation] and [[cite:citation]] formats)
        const citationMatches =
          content.match(/\[@[\w-]+\]|\[\[cite:[\w-]+\]\]/g) || [];
  ...
  getNodeLinks(id: string): string[] {
    ...
    // biome-ignore lint/suspicious/noAssignInExpressions: Required for regex matching loop
  ...
  getNodeBacklinks(_id: string): NodeInfo[] {
  ...
  parseNode(id: string): { metadata: unknown; content: string } | null {
    const node = this.findFileById(id);
    if (!node) {
      return null;
    }
