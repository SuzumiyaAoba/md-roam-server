...
search.get("/:query", async (c) => {
  try {
    ...
    if (!query || query.trim() === "") {
      return errorResponse(c, "Search query is required", undefined, 400);
    }
...
search.get("/tags/:tag/nodes", async (c) => {
  try {
...
search.get("/aliases/:alias/nodes", async (c) => {
  try {
...
search.get("/refs/:ref/nodes", async (c) => {
  try {
...
search.get("/citations/:citation/nodes", async (c) => {
  try {
