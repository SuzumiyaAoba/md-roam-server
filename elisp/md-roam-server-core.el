...
(defun md-roam-server--create-default-config ()
  ...
  (with-temp-file md-roam-server-config-file
    ...
    (insert "server:\n")
    (insert "  port: 8080                    # Emacs server port (backend/internal)\n")
    (insert "                                # Hono API server uses port 3001 (main API)\n\n")
    (insert "org-roam:\n")
...
(defun md-roam-server--apply-config ()
  ...
  (let ((config (md-roam-server--load-config)))
    ...
    (message "Configuration loaded: API port=%d, org-roam directory=%s, Japanese=%s, UTF-8=%s" 
             md-roam-server-port 
             org-roam-directory
